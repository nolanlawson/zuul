...
var db = new PouchDB(process.env.COUCH_URL, !process.env.COUCH_USERNAME ? {} : {
  auth: {
...
module.exports = function(config) {
    ...
    app.post('/__zuul/results', bodyParser.json(), function (req, res) {
      db.put(req.body).then(function () {
        res.status(201).send();
      }).catch(err => {
        console.log('error posting', err.stack);
        res.status(500).send();
      });
    });
